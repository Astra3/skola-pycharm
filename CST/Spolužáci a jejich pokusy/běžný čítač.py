...
class RST:
    ...
    def _runRSTr(self, S: int, R: int, C: int, Reset: int) -> list:
...
class D(RST):
    ...
    def _runDr(self, D: int, C: int, Reset: int) -> list:
        """Obvod D napsaný tak, aby fungoval s T.runTr"""
        self.Q, self.nQ = self._runRSTr(D, not D, C, Reset)
        return [self.Q, self.nQ]
...
class T(D):
    ...
    def runTr(self, C: int, Reset: int) -> list:
        ...
        :return: Vrací Q i nQ v listu
        """
        self.Q, self.nQ = self._runDr(self.nQ and Reset, C, Reset)
        return [self.Q, self.nQ]
t0 = T()
t1 = T()
t2 = T()
reset = 1
while True:
    Y0 = t0.runTr(int(input("C: ")), reset)[0]  # value vrací hodnotu tlačítka, pokud zmáčknuté tak je 1
